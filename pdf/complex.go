...
func Complex1Report(name string) {
	r := gr.CreateGoReport()
	//Page Total Function
	r.PageTotal = true
	...
	font1 := gr.FontMap{
		FontName: "IPAexG",
		FileName: "pdf//ttf//ipaexg.ttf",
	}
	font2 := gr.FontMap{
		FontName: "MPBOLD",
		FileName: "pdf//ttf//mplus-1p-bold.ttf",
	}
	...
	r.Records = gr.ReadTextFile("pdf/invoice.txt", 12)
	...
	r.Execute(fmt.Sprintf("out/%s.pdf", name))
}
...
func (h C1Header) Execute(report gr.GoReport) {
	...
	y := 32.0
	if report.SumWork["g2item"] == 0.0 {
		report.Image("pdf/nexuzhealth.jpg", 20, 35, 35, 50)
